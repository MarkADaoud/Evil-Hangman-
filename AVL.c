...
int get_balance_factor(Node *root)
{
	...
	return get_depth(root->right) - get_depth(root->left);
}
...
Node *rotate_right(Node *y)
{
	...
	// Update depths
	y->depth = max(get_depth(y->left), get_depth(y->right)) + 1;
	x->depth = max(get_depth(x->left), get_depth(x->right)) + 1;
...
Node *insert_new_node(Node *root, MY_STRING key, MY_STRING word)
{
	...
	// Update the depth of the ancestor node
	root->depth = 1 + max(get_depth(root->left), get_depth(root->right));
